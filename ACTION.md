...
Ten production issues identified and fixed across the backend, SDK, and configuration:

- **CORS:** Removed placeholder `your-vercel-app.vercel.app` origin; simplified origins to `localhost` + `settings.frontend_url`.
- **Secrets:** Removed insecure default values for `SECRET_KEY`, `JWT_SECRET`, `GITHUB_CLIENT_ID`, and `GITHUB_CLIENT_SECRET` in `config.py` — all are now required fields.
- **OAuth State:** Migrated in-memory `oauth_states` dict to a persistent `OAuthState` database table (`models.py`, `auth.py`), surviving restarts and multi-worker deployments.
- **Debug:** Changed `debug` default from `True` to `False` in `config.py`.
- **Registry URL:** `cli.py` now imports `DEFAULT_REGISTRY_URL` from `auth_config.py` instead of duplicating the constant.
- **Sandbox:** Removed `getattr` from the code-execution sandbox safe builtins in `runner.py` to prevent escape via attribute chain traversal.
- **Startup:** Replaced deprecated `@app.on_event("startup")` with the modern `lifespan` async context manager in `main.py`.
- **Health Check:** `/health` now only exposes `environment` when `debug=True`.
- **`.env.example`:** Updated to include all production variables: `ENVIRONMENT`, `DEBUG`, `SECRET_KEY`, `DATABASE_URL`, `JWT_SECRET`, `JWT_EXPIRE_HOURS`.

## Session Update — 2026-03-28 (Standalone CLI & Docs Sync)

- Published `ai-skills-sdk` to PyPI — users can now `pip install ai-skills-sdk` directly.
- Updated all markdown files (`README.md`, `OVERVIEW.md`, `CONTRIBUTING.md`, `registry/README.md`, `registry/api/README.md`, `docs/LAUNCH_POST.md`) to reference PyPI install as primary method.
- Added `aiskills login` step and `--dry-run` flag to all publish guides.
- Updated `OVERVIEW.md` tech stack: moved registry backend, frontend, auth, and hosting from "Planned" to "Current".
- Updated `OVERVIEW.md` CLI commands reference: moved `publish`, `install`, `login` from "Coming soon" to "Already available".
